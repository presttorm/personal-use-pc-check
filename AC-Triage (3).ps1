...
.PARAMETER SaveReport
    If specified, writes the full findings, timeline, and summary to local files
    (.txt and .json) instead of only printing to the console. Nothing leaves the
    machine - files are written to -ReportPath for you to review, archive, or
    share manually on your own terms.

.PARAMETER ReportPath
    Directory where report files are saved when -SaveReport is used.
    Defaults to the current user's Desktop.

.EXAMPLE
    .\AC-Triage.ps1
    Runs the triage and prints results to the console only.

.EXAMPLE
    .\AC-Triage.ps1 -SaveReport
    Runs the triage, prints to console, and also saves AC-Triage-Report-<timestamp>.txt
    and .json to the Desktop.

.EXAMPLE
    .\AC-Triage.ps1 -SaveReport -ReportPath "C:\Investigations\Case001"
    Saves the report files to a specific folder.
...
# -----------------------------------------------------------------------------
#  OPTIONAL: SAVE REPORT TO DISK  (only runs if -SaveReport was passed)
# -----------------------------------------------------------------------------
# This writes everything locally for the user to review and share manually.
# Nothing is ever sent over the network by this script.
if ($SaveReport) {
    try {
        if (-not (Test-Path $ReportPath -ErrorAction SilentlyContinue)) {
            New-Item -Path $ReportPath -ItemType Directory -Force -ErrorAction Stop | Out-Null
        }

        $timestamp  = Get-Date -Format "yyyyMMdd-HHmmss"
        $txtFile    = Join-Path $ReportPath "AC-Triage-Report-$timestamp.txt"
        $jsonFile   = Join-Path $ReportPath "AC-Triage-Report-$timestamp.json"

        # ---- Build the plain-text report ----
        $txtLines = [System.Collections.Generic.List[string]]::new()
        $txtLines.Add("================================================================")
        $txtLines.Add(" AC-Triage Forensic Report")
        $txtLines.Add(" Generated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')")
        $txtLines.Add(" Host: $env:COMPUTERNAME | User: $env:USERNAME")
        $txtLines.Add(" Analysis duration: $([Math]::Round($elapsed.TotalSeconds,1))s")
        $txtLines.Add("================================================================")
        $txtLines.Add("")
        $txtLines.Add("OVERALL RISK ASSESSMENT: $overallRisk")
        $txtLines.Add("")
        $txtLines.Add("Finding counts: Critical=$critCount High=$highCount Medium=$medCount Low=$lowCount Informational=$infoCount Total=$totalFindings")
        $txtLines.Add("Collection errors: $($script:CollectionErrors.Count)")
        $txtLines.Add("")
        $txtLines.Add("----------------------------------------------------------------")
        $txtLines.Add(" FINDINGS (grouped by category, highest confidence first)")
        $txtLines.Add("----------------------------------------------------------------")

        foreach ($cat in $categories) {
            $catFindings = $script:Findings | Where-Object { $_.Category -eq $cat } |
                Sort-Object { switch ($_.Confidence) { 'Critical' {0} 'High' {1} 'Medium' {2} 'Low' {3} default {4} } }
            $txtLines.Add("")
            $txtLines.Add("== $cat ==")
            foreach ($f in $catFindings) {
                $txtLines.Add("")
                $txtLines.Add("[$($f.Confidence)] $($f.Title)")
                $txtLines.Add("  Timestamp:  $($f.Timestamp)")
                $txtLines.Add("  Evidence:   $($f.Evidence)")
                $txtLines.Add("  Why:        $($f.WhySuspicious)")
                $txtLines.Add("  Benign alt: $($f.BenignExplanation)")
                if ($f.FollowUp) { $txtLines.Add("  Follow-up:  $($f.FollowUp)") }
            }
        }

        $txtLines.Add("")
        $txtLines.Add("----------------------------------------------------------------")
        $txtLines.Add(" UNIFIED TIMELINE")
        $txtLines.Add("----------------------------------------------------------------")
        foreach ($event in ($script:Timeline | Sort-Object Time)) {
            $txtLines.Add("[$($event.Time.ToString('yyyy-MM-dd HH:mm:ss'))] [$($event.Source)] $($event.Event)$(if ($event.Detail) { ' -- ' + $event.Detail } else { '' })")
        }

        if ($script:CollectionErrors.Count -gt 0) {
            $txtLines.Add("")
            $txtLines.Add("----------------------------------------------------------------")
            $txtLines.Add(" COLLECTION ERRORS (non-fatal)")
            $txtLines.Add("----------------------------------------------------------------")
            foreach ($err in $script:CollectionErrors) { $txtLines.Add("  $err") }
        }

        [System.IO.File]::WriteAllLines($txtFile, $txtLines, [System.Text.Encoding]::UTF8)
        ...
        $jsonReport = [ordered]@{
            ...
            Timeline         = ($script:Timeline | Sort-Object Time | ForEach-Object {
                @{ Time = $_.Time.ToString('yyyy-MM-dd HH:mm:ss'); Source = $_.Source; Event = $_.Event; Detail = $_.Detail }
            })
            CollectionErrors = $script:CollectionErrors
        }
        $jsonReport | ConvertTo-Json -Depth 6 | Out-File -FilePath $jsonFile -Encoding UTF8

        Write-Host ""
        Write-Host "  Report saved locally (nothing was sent over the network):" -ForegroundColor Green
        Write-Host "    $txtFile" -ForegroundColor White
        Write-Host "    $jsonFile" -ForegroundColor White
        Write-Host ""
        Write-Host "  Share these files yourself if/when you choose to." -ForegroundColor Gray
